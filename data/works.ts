...
export const works: Work[] = [
  ...
  },
  {
    ...
    technologies: ["SwiftUI", "EPS32", "BLE"],
    link: "https://www.turnie.io/",
    details: {
      overview: {
        ja: "turnieは、デジタル疲れが蔓延する現代において、人間同士のつながりを再定義するチャーム型の通信デバイス。物理的にすれ違ったユーザー同士で「tune」と呼ばれる8×8ピクセルのメッセージを自動的に交換する。チャットやフォロー、通知といった機能をあえて排除することで、偶然の出会いがもたらす静かな高揚感に焦点を当て、アルゴリズム主導のSNSに対するアンチテーゼを提示している。",
        en: "turnie is a charm-sized communication device designed to reimagine human connection in an era of digital fatigue. It automatically swaps 8×8 pixel messages, known as 'tunes,' with other users in physical proximity. By intentionally removing features like chat, follows, and notifications, it focuses on the 'quiet thrill' of coincidental encounters, serving as a physical antithesis to algorithm-driven social interactions.",
      },
      concept: {
        ja: "コンセプトは「すれ違いの共感（Passing empathy）」。デジタル上の足跡やSNSの指標に縛られることなく、同じ時間、同じ場所に誰かがいたという「偶然の一致」が持つ魔法を取り戻すことを目指している。現代の過剰なオンライン接続から解放されつつ、予期せぬ出会いに対して心を開く姿勢を形にしたプロダクトである。",
        en: "'Passing empathy.' The project aims to bring back the magic of true coincidence—being in the same place at the same time—without the pressure of digital footprints or social media metrics. It is a statement of being open to the unexpected while remaining free from the constraints of modern online connectivity.",
      },
    },
  },
  {
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "「AIが答えを出す時代に、⼈間の思考を守るツール」として、⽣成AIが何もかも⽣成する時代に考えることにフォーカスしたアプリケーション。ローカルファイルを解析して意味ベクトルを作成、3D空間に表⽰することで、⾃分のメモや⽇記などの記録を「意味」をもとに閲覧・探索することを可能にした。⾃分の思考の偏りや分布を知ることで思索を深めるためのツール。第1回Hack1グランプリのために作成され、Designship賞を受賞した。",
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "3D空間を使用した写真アルバムとしてのWebサイト。平面グリッドが一般的なアルバムに対し、XYZ方向に写真を配置し、自由にズームイン・アウト、クリックによる詳細閲覧が可能。",
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "Spotify APIを使用したSpotify上での視聴履歴の分析が可能なWebページ。余分なエレメントを排した、必要十分な構成とした。",
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "種田山頭火の詩集「草木塔」を読むためのWebサイト。機能はスクロールおよびタッチによる前後への移動のみ。詩の情報は青空文庫より引用した。",
      ...
      concept: {
        ja: "自身が好きな詩集を気軽に、余計な情報を排して読みたいという動機から制作。操作は主に1つずつ閲覧する方法を取っているが、700を超える詩を自由に行き来できるようにRandomボタンを設置し、思いがけない場所に移動できるようになっている。localStorageに現在の位置を保存することで、再び訪問した際に前回の続きから読めるようにした。",
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "鑑賞した映画の情報や感想をNotionで管理するためのChrome Extension。IMDbの映画ページから情報を取得し、ポップアップウィンドウからコメントやレーティングをNotionに自動で記録することができる。",
      ...
      concept: {
        ja: "IMDbや他のサービスが提供する映画鑑賞記録用のサービスでは、他ユーザーの情報がノイズになったり、自分が残したい情報が記録できなかったりと不自由が多かった。そこで、Notionのデータベースに記録するという形で情報の操作性と自由度の高い映画鑑賞記録用のシステムを作成した。",
  ...
  },
  {
    ...
    details: {
      ...
      concept: {
        ja: "本Webサイトは制作から公開までの期間が短かったことから、デザインと開発を同時に一人で行った。キービジュアルと展示のコンセプトを考慮してデザインを行い、スマートフォンでの見やすさも考慮してレスポンシブに重きをおいて開発した。",
        en: "Since the period from production to release of this website was short, I designed and developed it at the same time, by myself. I designed it considering the key visual and the concept of the exhibition, and developed it with a focus on responsiveness, considering the ease of viewing on smartphones without compromising visual quality.",
      },
    },
  },
  {
    ...
    details: {
      overview: {
        ja: "本Webサイトは東京都立大学の授業「ネットワーク実習I」の履修者たちの制作課題をまとめた作品集としてのZINE『もじとおどる - JavaScriptで楽しむWebと文字』の紹介ページとして制作された。ZINEの内容を紹介するとともに、購入ページとしても機能する。",
      ...
      concept: {
        ja: "制作したZINEを紹介するためのページであるため、情報の完結性と購入ページへのアクセスを容易にした。掲載作品はWebで動作するものであるため、本サイトからアクセスできるように誌面と連動した掲載方法を取った。",
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "Gundiは新しいコミュニケーションの形を提案するチャットサービス。絵文字のみや漢字1文字のみなど、制限された手段のなかで、従来の加速型のコミュニケーションとは異なる、ゆったりとしたコミュニケーションを実現する。この作品はCCBT(Civic Creative Base Tokyo)で行われたFuture Ideations Camp Vol.2 setup()で制作したconVerseを軸に発展させて制作された。",
      ...
      concept: {
        ja: "本サービスは、負の感情により加速度を増すSNSの現状に着目し、感情を加速させない、ゆったりとしたコミュニケーションを実現することを目的としている。制限されたコミュニケーション手法と平面的に展開されるインターフェースが特徴。絵文字や漢字1文字などの極度に制限された状況下でのコミュニケーションは、必ずしも情報を伝達することに特化しておらず、このチャットルームにおいて「誰かと存在している」ことを実感することを目指している。また、平面的に展開されるインターフェースは、一般的なスレッド型でなく平面に付箋を置くように2次元的にチャットを行うことができ、各メッセージの距離感や位置などを意識することで、よりリアルなコミュニケーションを実現する。これら2つの特徴により、本サービスは従来のコミュニケーションツールとは異なる、新しいコミュニケーションの形を提案する。",
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "conVerseは新しいコミュニケーションの形を提案するチャットサービス。絵文字のみや漢字1文字のみなど、制限された手段のなかで、従来の加速型のコミュニケーションとは異なる、ゆったりとしたコミュニケーションを実現する。この作品はCCBT(Civic Creative Base Tokyo)で行われたFuture Ideations Camp Vol.2 setup()で制作され、「Gundi」と名前を変えてプロジェクトを継続している。",
  ...
  },
  {
    ...
    details: {
      overview: {
        ja: "本Webサイトは東京都立大学の大学祭で開催されたインタラクティブアートの展示「有機的、無機的」の広報のために制作された。バックグラウンドにはThree.jsとWebGLを用いて、有機的な形状と無機的な形状が溶け合う様子を描画している。",
      ...
      concept: {
        ja: "展示の軸である「有機的、無機的」を、展示内容とは異なるビジュアルでありながらも一目でタイトルとの一貫性を持たせることを目指した。また、ファーストビューの文字情報を大胆に配置することで、Webサイトでありながらもポスター的な存在感を持たせることを意識した。",
